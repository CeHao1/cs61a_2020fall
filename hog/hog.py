...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        a.append(dice())
    return sum(a) if min(a)!=1 else 1    
    # END PROBLEM 1
...
def swine_align(player_score, opponent_score):
    ...
    if n==0:
        return False
    while player_score % n != 0 or opponent_score %n != 0 :
        n-=1
    return n>=10
    # END PROBLEM 4a
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    return player_score<opponent_score and player_score+3>opponent_score
    # END PROBLEM 4b
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(s0,s1):
        ...
        comp=(s1 if who==1 else s0)-ls
        if comp>rh:
            print(comp,'point(s)! The most yet for Player',who)
            rh=comp
        ls+=comp
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    return 0 if fb>=cutoff or extra_turn(fb+score,opponent_score) else num_rolls
    # END PROBLEM 11
